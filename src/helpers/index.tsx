...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const leaf_usdc_address = addresses[networkID].LEAF_USDC_LP_ADDRESS;
  const pairContract = new ethers.Contract(leaf_usdc_address, PairContract, provider);
  const reserves = await pairContract.getReserves();
  const marketPrice = reserves[1] / reserves[0] * Math.pow(10, 12);
